...
function xmldb_videoassessment_upgrade($oldversion = 0) {
    ...
    if ($oldversion < 2015032010) {
        ...
        $DB->delete_records('grade_items', ['itemnumber' => 4]);
        ...
        foreach ($courses as $course) {
            ...
            foreach ($users as $user) {
                ...
                if ($grade->count > 0) {
                    ...
                    $itemgrade = $DB->get_record('grade_grades', [
                        'itemid' => $courseitem->id,
                        'userid' => $user->id,
                    ]);

                    if (!empty($itemgrade)) {
                        $itemgrade->finalgrade = $grade->total / $grade->count;
                        $DB->update_record('grade_grades', $itemgrade);
                    }
                }
            }
        }
    ...
    if ($oldversion < 2016040700) {
        ...
        $table->add_key('primary', XMLDB_KEY_PRIMARY, ['id']);
        $table->add_key('sortitemid', XMLDB_KEY_FOREIGN, ['sortitemid'], 'videoassessment_sort_items', ['id']);
        ...
        try {
            ...
            if (!empty($ues)) {
                foreach ($ues as $ue) {
                    $sortitem = $DB->get_record('videoassessment_sort_items', ['type' => 'course', 'itemid' => $ue->courseid]);
            ...
            if (!empty($gms)) {
                foreach ($gms as $gm) {
                    $sortitem = $DB->get_record('videoassessment_sort_items', ['type' => 'group', 'itemid' => $gm->groupid]);
